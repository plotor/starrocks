...
public:
    ...
    arrow::Status open() override {
        ...
        _tablet_manager = std::make_shared<TabletManager>(provider, config::lake_metadata_cache_limit);
        ...
        FORMAT_ASSIGN_OR_RAISE_ARROW_STATUS(auto metadata,
                                            _tablet_manager->get_tablet_metadata(fs, metadata_location, true));
        ...
        _scan_schema =
                std::make_shared<starrocks::Schema>(ChunkHelper::convert_schema(_tablet_schema, _scan_column_indexes));
        ...
        if (std::equal(_scan_column_indexes.begin(), _scan_column_indexes.end(), output_column_indexes.begin(),
            ...
            _output_tablet_schema = std::make_shared<starrocks::Schema>(
                    ChunkHelper::convert_schema(_tablet_schema, output_column_indexes));
    ...
    arrow::Status get_next(struct ArrowArray* c_arrow_array) override {
        ...
        if (record_batch) {
            return arrow::ExportRecordBatch(*record_batch, c_arrow_array);
        } else {
            return arrow::Status::OK();
        }
    }
...
private:
    ...
    arrow::Result<int32_t> get_column_id(const std::shared_ptr<arrow::Field>& field) {
        ...
        if (!field->HasMetadata() || !metadata->Contains("starrocks.columnId")) {
            return arrow::Status::Invalid("Invalid field unique id");
        }

        try {
            ARROW_ASSIGN_OR_RAISE(string str_unique_id, metadata->Get("starrocks.columnId"));
            unique_id = std::stoi(str_unique_id);
...
arrow::Result<StarRocksFormatReader*> StarRocksFormatReader::create(
        ...
        std::shared_ptr<arrow::Schema> output_schema, std::string tablet_root_path,
        std::unordered_map<std::string, std::string> options) {
    StarRocksFormatReaderImpl* format_Reader =
            new StarRocksFormatReaderImpl(tablet_id, version, std::move(required_schema), std::move(output_schema),
                                          std::move(tablet_root_path), std::move(options));
