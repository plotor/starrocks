...
namespace starrocks::lake::format {
    ...
    public:
        StarRocksFormatReaderImpl(int64_t tablet_id,
                                  int64_t version,
            ...
            auto itr = _options.find("starrocks.format.chunk_size");
        ...
        arrow::Status open() override {
            ...
            _scan_schema =
                    std::make_shared<starrocks::Schema>(
                            ChunkHelper::convert_schema(_tablet_schema, _scan_column_indexes));
            ...
            if (std::equal(_scan_column_indexes.begin(), _scan_column_indexes.end(), output_column_indexs.begin(),
                ...
                LOG(INFO) << "OutputSchema and ScanSchema are not same. OutputSchema size is "
                          << output_column_indexs.size() << " and ScanSchema size is " << _scan_column_indexes.size()
                          << ".";
        ...
        void close() override {
            if (_tablet_reader) {
                ...
                LOG(INFO) << "Close tablet reader with "
                          ...
                          << _tablet_reader->stats().compressed_bytes_read_request
    ...
    private:
        ...
        Status parse_query_plan(std::string &encoded_query_plan) {
            ...
            RETURN_IF_ERROR(
                    DescriptorTbl::create(_state.get(), &_obj_pool, t_query_plan_info.desc_tbl, &_desc_tbl, 4096));
            ...
            for (auto slot: _tuple_desc->slots()) {
                ...
                auto it = std::find(_scan_column_indexes.begin(), _scan_column_indexes.end(), index);
                if (it != _scan_column_indexes.end()) {
            ...
            if (plan_node->__isset.conjuncts && plan_node->conjuncts.size() > 0) {
                RETURN_IF_ERROR(
                        Expr::create_expr_trees(&_obj_pool, plan_node->conjuncts, &_conjunct_ctxs, _state.get()));
        ...
        Status init_reader_params(TabletReaderParams &params) {
            ...
            OlapScanConjunctsManagerOptions conjuncts_manager_opts = OlapScanConjunctsManagerOptions{
            ...
            _conjuncts_manager = std::make_unique<OlapScanConjunctsManager>(std::move(conjuncts_manager_opts));
    ...
    arrow::Result<StarRocksFormatReader *> StarRocksFormatReader::create(
            ...
            std::unordered_map<std::string, std::string> options) {
        ARROW_ASSIGN_OR_RAISE(std::shared_ptr<arrow::Schema> required_schema,
                              arrow::ImportSchema(required_arrow_schema));
        ARROW_ASSIGN_OR_RAISE(std::shared_ptr<arrow::Schema> output_schema, arrow::ImportSchema(output_arrow_schema));
        return create(tablet_id, version, required_schema, output_schema, std::move(tablet_root_path),
                      std::move(options));
    }
