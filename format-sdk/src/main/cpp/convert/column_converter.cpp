...
#undef CONVERTER_CASE_IMPL
    ...
    arrow::Result<ColumnConverterVector> ColumnConverter::create_children_converter(
        ...
        if (arrow_type->id() == arrow::Type::LIST && sr_field->type()->type() == starrocks::LogicalType::TYPE_ARRAY) {
            ...
            if (!sr_field->has_sub_fields() || sr_field->sub_fields().size() != 1) {
                return arrow::Status::Invalid("Invalid sub fields size for array field ", sr_field->name());
            }
            ...
            std::shared_ptr<starrocks::Field> sr_element_field = std::make_shared<starrocks::Field>(
                    sr_field->sub_field(0));

            ARROW_ASSIGN_OR_RAISE(auto converter,
                                  ColumnConverter::create(arrow_offset_datatype, sr_offset_field, pool));
            ...
            result.emplace_back(converter);
                   ...
                   sr_field->type()->type() == starrocks::LogicalType::TYPE_MAP) {
            ...
            for (size_t idx = 0; idx < arrow_type->num_fields(); ++idx) {
                if (arrow_type->field(idx)->name() != sr_field->sub_field(idx).name()) {
                    return arrow::Status::Invalid("child field's name not match with arrow:",
                                                  arrow_type->field(idx)->name(), ", sr:",
                                                  sr_field->sub_field(idx).name());
                }
    ...
    arrow::Result<std::shared_ptr<arrow::Buffer>> ColumnConverter::convert_null_bitmap(
            const Buffer<uint8_t> &null_bytes) {
    ...
    std::shared_ptr<starrocks::Column>
    ColumnConverter::get_data_column(const std::shared_ptr<starrocks::Column> &column) {
